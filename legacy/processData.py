...
# create dictionary to store img and annotations

data = {}
directory = 'train_images_h'
box_file = 'boxes_h.txt'

# get indexes of all images
image_indexes = [int(img_name.split('.')[0]) for img_name in os.listdir(directory)]

# shuffle indexes to get random train/test split sets
...
# Convert bounding boxes to dictionary in format 'index:(x1,y1,x2,y2)'
...
for index in image_indexes:
    # read img in memory and append it to the list
    img = cv2.imread(os.path.join(directory, str(index) + '.png'))

    # read associated bounding box
...
print('Number of Images and Boxess Present: {}'.format(len(data)))
